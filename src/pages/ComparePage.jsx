        ...
        <table className="compare-table">
          <thead>
            <tr>
              ...
              {items.map(p => (
                <th key={p.id} style={{ textAlign: 'center', minWidth: 220 }}>
                  <div style={{ fontSize: 40, marginBottom: 8 }}>{p.emo}</div>
                  ...
                  <button className="btn btn-ghost btn-xs" style={{ marginTop: 8 }} onClick={() => remove(p.id)}>Remove</button>
                </th>
              ))}
            </tr>
          </thead>
            ...
            <tr>
              ...
              {items.map(p => (
                <td key={p.id} style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--mute)' }}>{p.rating} ({p.reviews.toLocaleString()})</div>
                </td>
              ))}
            </tr>
            {allKeys.map(k => (
              <tr key={k}>
                ...
                {items.map(p => {
                  ...
                  return <td key={p.id} style={{ textAlign: 'center', fontWeight: 600 }}>{spec ? spec.v : '—'}</td>;
                })}
              </tr>
            ))}
            <tr>
              ...
              {items.map(p => (
                <td key={p.id} style={{ textAlign: 'center' }}>
                  <button className="btn btn-accent btn-sm" onClick={() => { addToCart(p); toast(`${p.name} added to cart`); }}>Add to Cart</button>
                </td>
              ))}
            </tr>
          </tbody>
